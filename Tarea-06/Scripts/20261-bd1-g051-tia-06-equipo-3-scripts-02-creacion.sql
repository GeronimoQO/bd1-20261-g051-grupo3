...
CREATE TABLE consumidor (
    consumidor_id  INTEGER      GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ...
    CONSTRAINT fk_consumidor_ubicacion
        FOREIGN KEY (ubicacion_id)
        REFERENCES ubicacion (ubicacion_id)
        ON UPDATE CASCADE ON DELETE SET NULL
);
...
CREATE TABLE mercado (
    mercado_id     INTEGER      GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    nombre_mercado VARCHAR(150) NOT NULL,
    direccion      VARCHAR(255),
    tipo_mercado   VARCHAR(80)  NOT NULL DEFAULT 'Físico',
    activo         BOOLEAN      NOT NULL DEFAULT TRUE,
    ubicacion_id   INTEGER      NOT NULL UNIQUE,

    CONSTRAINT fk_mercado_ubicacion
        FOREIGN KEY (ubicacion_id)
        REFERENCES ubicacion (ubicacion_id)
        ON UPDATE CASCADE ON DELETE RESTRICT
);

-- 8. TABLA: usuario_apiario
...
CREATE TABLE pedido (
    ...
    CONSTRAINT fk_pedido_mercado
        FOREIGN KEY (mercado_id)
        REFERENCES mercado (mercado_id)
        ON UPDATE CASCADE ON DELETE RESTRICT
);

-- 17. TABLA: transaccion
...
CREATE TABLE detalle_pedido (
    ...
    CONSTRAINT pk_detalle_pedido
        PRIMARY KEY (pedido_id, publicacion_id),
...
-- 20. TABLA: transaccion_publicacion
